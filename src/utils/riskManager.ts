...
import { Portfolio, Position, Signal, RiskCheck, RiskViolation } from '../types';
...
export class RiskManager {
  ...
  private lastDailyLoss: number = 0;
  private lastWeeklyLoss: number = 0;
  ...
  checkSignal(signal: Signal, portfolio: Portfolio): RiskCheck {
    ...
    if (!passed) {
      logger.warn('Risk check failed', { violations });
    }
   ...
   */
  private checkPositionLimit(signal: Signal, portfolio: Portfolio): RiskViolation | null {
    ...
    const maxPositionPercent = config.risk.maxPositionPerAsset;
   ...
   */
  private checkLeverageLimit(portfolio: Portfolio): RiskViolation | null {
   ...
   */
  private checkDrawdown(portfolio: Portfolio): RiskViolation[] {
    const violations: RiskViolation[] = [];
    const now = Date.now();
    ...
    if (dailyLoss < -config.risk.dailyLossLimit) {
      violations.push({
        type: 'drawdown',
        severity: 'critical',
        message: `Daily loss ${(Math.abs(dailyLoss) * 100).toFixed(1)}% exceeds limit ${(config.risk.dailyLossLimit * 100).toFixed(1)}%`,
    ...
    if (weeklyLoss < -config.risk.weeklyLossLimit) {
      violations.push({
        type: 'drawdown',
        severity: 'critical',
        message: `Weekly loss ${(Math.abs(weeklyLoss) * 100).toFixed(1)}% exceeds limit ${(config.risk.weeklyLossLimit * 100).toFixed(1)}%`,
        current: Math.abs(weeklyLoss),
        limit: config.risk.weeklyLossLimit,
      });
    }

    // Check total drawdown
    if (drawdown > config.risk.totalLossLimit) {
      violations.push({
        type: 'drawdown',
        severity: 'critical',
        message: `Total drawdown ${(drawdown * 100).toFixed(1)}% exceeds limit ${(config.risk.totalLossLimit * 100).toFixed(1)}%`,
        current: drawdown,
        limit: config.risk.totalLossLimit,
      });
    }
   ...
   */
  private checkEmergencyReserve(portfolio: Portfolio): RiskViolation | null {
    const requiredReserve = portfolio.totalValueUsd * config.risk.emergencyReservePercent;
  ...
  getRiskStatus(portfolio: Portfolio): {
    ...
    return {
      leverage,
      maxPositionUtilization: maxPositionPercent / config.risk.maxPositionPerAsset,
